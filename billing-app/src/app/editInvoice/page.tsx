...
function EditInvoiceContent() {
    ...
    const fetchData = async () => {
      try {
        const [buyersRes, productsRes] = await Promise.all([
          fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/buyers`),
          fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/products`)
        ]);
  ...
  useEffect(() => {
    ...
    fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/invoices/${invoiceId}`)
      .then(res => res.json())
      .then(data => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/invoices/${invoice._id}`,
        {
          method: 'PUT',
