...
export default function CreateInvoicePage() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/buyers`).then(res => res.json()),
      fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/products`).then(res => res.json())
    ])
  ...
  const saveInvoice = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/invoices`, {
        method: 'POST',
