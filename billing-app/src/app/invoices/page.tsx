...
export default function InvoiceListPage() {
  ...
  useEffect(() => {
    setLoading(true);
    fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/invoices`)
      .then((res) => {
        if (!res.ok) {
  ...
  try {
    ...
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/export-invoice-pdf`, {
      method: 'POST',
  ...
  const deleteInvoice = async (invoiceId: string, invoiceNo: string) => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/invoices/${invoiceId}`, {
        method: 'DELETE',
      });
        ...
        <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 sm:px-6 sm:py-4 rounded-lg sm:rounded-xl shadow-sm">
          <div className="flex items-center space-x-2 sm:space-x-3">
            ...
            <div>
              ...
              <p className="text-xs sm:text-sm text-red-600 mt-1">Please ensure your API server is running</p>
            </div>
          </div>
        </div>
      ) : invoices.length === 0 ? (
                      ...
                      <div className="flex flex-col gap-1 sm:gap-2 flex-shrink-0">
                        ...
                        {inv.status !== 'paid' && (
                          <button
                            ...
                            onClick={async (e) => {
                              e.stopPropagation();
                              await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/invoices/${inv._id}/status`, {
                                method: 'PUT',
                                headers: { 'Content-Type': 'application/json' },
